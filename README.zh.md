...
| **Timeline Editor** | 全屏多轨时间轴编辑器；输出 `data_json` 与 `frame_seq_dir` | [→](docs/zh/audio-timeline.md) |
...
Timeline Editor / Audio Timeline 的 **禁用 / 启用** 功能让你可以只重新生成某一段，而无需修改其他部分——禁用其余片段，重新运行，再恢复即可。详见 [Audio Timeline 文档](docs/zh/audio-timeline.md#片段禁用--启用)。
